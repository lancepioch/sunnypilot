...
if __name__ == "__main__":
  ...
  if is_unified_supercombo:
    ...
    for cam_w, cam_h in set(args.camera_resolutions):
      ...
      def cleanup_unified():
        global run_model_jit, run_policy, model_runner, warp
        run_model_jit, run_policy, model_runner, warp = None, None, None, None

      compiled_jit = compile_jit(
        run_model_jit, stock.MODELD_INPUTS, make_model_queues, benchmark_runs=args.benchmark_runs, clear_refs=cleanup_unified
      )
      tmp_f = tempfile.NamedTemporaryFile(delete=False, dir=".")
      dump_oob(compiled_jit, tmp_f)
      tmp_f.close()
      run_model_tmp_files[(cam_w, cam_h)] = tmp_f.name
      del compiled_jit
      gc.collect()
    for k, path in run_model_tmp_files.items():
      with open(path, "rb") as f:
        output_data['run_model'][k] = load_oob(f)
      os.remove(path)
    ...
    for cam_w, cam_h in set(args.camera_resolutions):
      ...
      output_data[(cam_w, cam_h)] = compile_jit(warp, WARP_INPUTS, make_warp_queues, make_random_inputs=make_random_warp_inputs,
                                                benchmark_runs=args.benchmark_runs, clear_refs=cleanup_warp)
